...
class BalishijiaEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            "up": [
                "🏠 {nickname} 住进巴黎牛家！妖牛市看好！",
                "🏠 硬度+10！牛价跟着硬起来！",
                "🏠 {nickname} 的硬度提升振奋了市场！",
                "🏠 「巴黎牛家是硬实力」—— 牛评家",
            ],
            "down": [
                "🏠 {nickname} 巴黎牛家入住失败！妖牛市失望！",
                "🏠 硬度换长度...妖牛市：这波亏了！",
                "🏠 {nickname} 的操作让牛民迷惑！",
                "🏠 「长度换硬度不划算」—— 牛评家",
            ],
...
class YueyaTianchongEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            ...
            "down": [
                "🌙 {nickname} 月牙天冲失手！妖牛市受伤！",
                "⚔️ 一刀两断！牛价也断了！",
                "🌙 月光之力失控！妖牛市暴跌！",
                "⚔️ {nickname} 的剑气误伤了妖牛市！",
                "🌙 「月牙天冲是熊市信号」—— 牛评家",
                "⚔️ 剑光过处，牛价跳水！",
            ],
        }
...
class FantanEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            "up": [
                "🔄 反弹护盾激活！妖牛市被反弹的气势感染 {change}",
                "🛡️ {nickname} 开启反弹模式，妖牛市跟着弹了 {change}",
            ],
            "down": [
                "🔄 反弹护盾就位，但妖牛市反向波动 {change}",
                "🛡️ {nickname} 准备反弹，妖牛市却先跌为敬 {change}",
            ]
        }
    }
...
class ShangbaoxianEffect(ItemEffect):
    ...
    # 妖牛市配置 - 工具类道具，微波动+平淡文案
    stock_config = {
        "volatility": (0.001, 0.005),
        "templates": {
            "plain": [
                "🛡️ {nickname} 买了保险，妖牛市反应平淡 {change}",
                "🛡️ 保险生效，妖牛市打了个哈欠 {change}",
                "🛡️ {nickname} 给牛牛上了保险，妖牛市：哦 {change}",
            ]
        }
    }
    ...
    stock_config = {
        ...
        "templates": {
            "up": [
                ...
                "🛡️ 防御姿态！妖牛市：稳住！",
                "🛡️ {nickname} 开启护盾模式，牛价跟着稳！",
                "🛡️ 牛牛有了保护，牛民也安心了！",
            ],
            "down": [
                "🛡️ {nickname} 献祭太多了！妖牛市心疼！",
                "🛡️ 防御代价太高！牛价跟着跌！",
                "🛡️ {nickname} 割肉买盾，妖牛市：这操作亏了！",
                "🛡️ 盾牌到手，但牛价没了！",
            ],
        }
    }
...
class NiuniuJishengEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            "up": [
                "🦠 {nickname} 种下寄生虫！妖牛市：这波血赚！",
                "🦠 寄生成功！被动收入让牛民羡慕！",
                "🦠 {nickname} 开启躺赚模式，牛价跟着涨！",
                "🦠 「寄生就是生产力」—— 牛评家",
            ],
            "down": [
                "🦠 {nickname} 种寄生虫...妖牛市：恶心！",
                "🦠 寄生行为引发市场反感！",
                "🦠 {nickname} 的寄生计划被妖牛市唾弃！",
                "🦠 「寄生虫是市场毒瘤」—— 牛评家",
            ],
        }
    }
...
class QuniuyaoEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            "plain": [
                "💊 {nickname} 用了驱牛药，妖牛市反应平淡 {change}",
                "💊 驱牛药生效，妖牛市打了个哈欠 {change}",
                "💊 {nickname} 清除了寄生虫，妖牛市：哦 {change}",
            ]
        }
    }
...
class HanxiaoWubudianEffect(ItemEffect):
    ...
    stock_config = {
        ...
        "templates": {
            "up": [
                "☠️ {nickname} 发动「含笑五步癫」！妖牛市为之震颤！",
                "💀 禁术「含笑五步癫」现世！牛价暴涨！",
                "🩸 {nickname} 倾家荡产的一击！妖牛市狂欢！",
            ],
            "down": [
                "☠️ {nickname} 发动「含笑五步癫」！妖牛市恐慌！",
                "💀 禁术「含笑五步癫」现世！牛价暴跌！",
                "🩸 {nickname} 的疯狂之举吓坏了妖牛市！",
            ],
        }
    }
    ...
    def on_trigger(self, trigger: EffectTrigger, ctx: EffectContext) -> EffectContext:
        ...
        if total_asset < HanxiaoWubudianConfig.MIN_ASSET:
            ctx.messages.extend([
                ...
                f"📊 你的总资产：{int(user_coins)}金币 + {int(stock_value)}妖牛券 = {int(total_asset)}",
        ...
        ctx.messages.extend([
            ...
            f"💸 自爆消耗：全部资产 {asset_consume:,}（金币{int(coins_to_deduct):,} + 妖牛券{shares_to_sell}股）",
            "",
