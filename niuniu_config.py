...
class CoinVanishConfig:
    ...
    # 格式：(最小比例, 最大比例, 概率)
    LOSS_TIERS = [
        (0.05, 0.10, 0.40),        # 5-10%损失：40%概率
        (0.15, 0.25, 0.30),        # 15-25%损失：30%概率
        (0.30, 0.40, 0.20),        # 30-40%损失：20%概率
        (0.45, 0.50, 0.10),        # 45-50%损失：10%概率
    ]
...
class RobberyConfig:
    ...
    ROBBERY_AMOUNT_TIERS = [
        (0.05, 0.15, 0.40),            # 5-15%：40%概率（小偷小摸）
        (0.20, 0.30, 0.30),            # 20-30%：30%概率（中等收获）
        (0.35, 0.50, 0.20),            # 35-50%：20%概率（大丰收）
        (0.55, 0.70, 0.10),            # 55-70%：10%概率（洗劫一空）
    ]
