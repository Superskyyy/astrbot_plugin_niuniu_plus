...
DEFAULT_SHOP_ITEMS = [
    ...
    },
    {
        ...
        'name': "牛牛寄生",
        'type': 'active',
        'desc': "在随机群友身上放置寄生牛牛！宿主每次获得>5%长度收益时，你吸取其5%长度和硬度！持续效果",
        'price': 150
    },
    {
        'id': 19,
...
class YueyaTianchongConfig:
    ...
    MAX_DAMAGE = 500                   # 伤害上限500cm，防止负数牛牛造成过高伤害
...
class NiuniuJishengConfig:
    MIN_PLAYERS = 2                # 最少需要2人（自己+1个目标）
    TRIGGER_THRESHOLD = 0.05       # 触发阈值：收益 > 宿主长度绝对值的5%
    DRAIN_LENGTH_PERCENT = 0.05    # 吸取长度百分比：宿主长度绝对值的5%
    DRAIN_HARDNESS_PERCENT = 0.05  # 吸取硬度百分比：宿主硬度的5%（向下取整，最低1）
    ...
    # 覆盖旧寄生的文案
    OVERRIDE_TEXTS = [
        "👊 你的寄生牛牛一脚踹走了{old_name}的寄生牛牛！",
        "⚔️ 你的寄生牛牛击败了{old_name}的寄生牛牛！",
        "🥊 你的寄生牛牛把{old_name}的寄生牛牛赶跑了！",
        "💥 你的寄生牛牛驱逐了{old_name}的寄生牛牛！",
        "🦵 {old_name}的寄生牛牛被你的寄生牛牛踢飞了！",
    ]

    # 吸血触发文案（宿主视角）
    DRAIN_TEXTS = [
        "🩸 你的{length}cm长度和{hardness}硬度被{name}的寄生牛牛吸走了！",
        "💉 {name}的寄生牛牛吸取了你{length}cm长度和{hardness}硬度！",
        "🧛 寄生牛牛发动！{name}吸走了你{length}cm和{hardness}硬度！",
        "😱 糟糕！{name}的寄生牛牛偷走了你{length}cm和{hardness}硬度！",
    ]
