...
DEFAULT_SHOP_ITEMS = [
    {
        ...
        'name': "含笑五步癫",
        'type': 'active',
        'desc': "消耗你95%总资产(含股票,底价10亿)，立即掠夺目标100%长度+硬度+资产！同时记录快照，目标之后每次主动行动都会损失快照值的19.6%，走5步共损失98%，效果无法被任何道具抵挡！用法：牛牛购买 0 @目标",
        'price': 0,
        'dynamic_price': True
    },
    {
...
#
# ==================== 购买代价 ====================
# 消耗规则：扣除购买者 95% 的总资产（金币 + 股票市值），最低 10亿
#   - 计算公式：cost = max(10亿, 总资产 × 95%)
#   - 扣除顺序：先扣金币，金币不足时强制卖出股票补足
#   - 股票强制清算：被清算的股票记为纯损失（不影响股价，无任何收益）
#
# ==================== 即时效果（对发起方）====================
# 立即获得目标快照的100%长度、硬度、资产（金币形式）
#
# ==================== 持续效果（对目标）====================
# 施加「含笑五步癫」debuff，不立即修改目标长度/硬度
#
# ==================== 含笑五步癫 机制 ====================
# 施加时记录快照：
...
class HanxiaoWubudianConfig:
    MIN_PLAYERS = 2                    # 最少需要2人（自己+1个目标）
    ASSET_CONSUME_PERCENT = 0.95       # 消耗95%总资产
    ...
    SUCCESS_TEXTS = [
        ...
        "🤪 {user} 消耗95%身家，对 {target} 使出禁术「含笑五步癫」！",
