...
DEFAULT_SHOP_ITEMS = [
    {
        ...
        'name': "化牛绵掌",
        'type': 'active',
        'desc': "消耗99%总资产(含股票持仓,底价10亿)！施加「化骨debuff」：目标每次行动都会损失被击时长度/硬度/总资产快照的24.5%（先扣币再卖股），共4次，无法抵挡！用法：牛牛购买 0 @目标",
        'price': 0,
        'dynamic_price': True
    },
    {
...
#
# 每次触发效果（共 4 次）：
#   - 长度损失 = snapshot_length × 24.5%（可变负）
#   - 硬度损失 = snapshot_hardness × 24.5%（最低归0）
#   - 资产损失 = snapshot_asset × 24.5%
#     - 先扣金币，金币不足时强制清算股票
#     - 股票强制清算记为纯损失（不影响股价）
#
# 触发条件：
...
#
# ==================== 排行榜和背包显示 ====================
# - 排行榜：有化骨debuff的玩家会显示 🦴 标识
# - 背包：显示化骨debuff详情（剩余次数、快照值）
#
# ==================== 设计理念 ====================
# 这是一个高风险高回报的终极打击道具：
# - 使用者需要付出巨大代价（99%总资产，最低10亿）
# - 目标会受到化骨debuff的持续伤害（4次 × 24.5%快照值）
# - 伤害无法被任何道具抵挡，让目标难以恢复
# - 适合用于报复或清算富牛
#
class HuaniuMianzhangConfig:
    ...
    # 成功使用文案
    SUCCESS_TEXTS = [
        "☠️ {user} 发动「化牛绵掌」！{target} 被一掌打入骨髓！",
        "💀 {user} 消耗99%身家，对 {target} 使出禁术「化牛绵掌」！",
        "🩸 {user} 倾家荡产，换来 {target} 的化骨诅咒！",
        "⚰️ {user} 燃烧一切！{target} 被「化牛绵掌」标记！",
    ]
