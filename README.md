...
### 3.3修复 遍历词典时数据错误的问题（可能还会有其他地方出现相同问题 有bug提交issues（**建议**）或加群@Superskyyy）
...
### 4.5
- 增加开冲日期判定，现在超过三次后应该不会再导致无法开冲了（
### 4.6
- 修改金币读取逻辑，修复和签到插件中显示金币不统一的问题，新增依赖文件，修改重置开冲逻辑,修复背包和打胶等功能数据不同步的问题（只是暂时改为调用main中方法修改，可能出问题，如果还有问题，16号之后修复）
### 4.7
- 修改数据读取和保存逻辑，现在数据都通过yml读取和保存，不会再出现数据回档以及金币异常（大概）
### 4.7.2 
- 对数据存读进行重构，解决数据回档？
## 四、经济系统
...
| 来源 | 频率 | 收益 |
|------|------|------|
| 开冲 | 3次/天，每次最长12小时 | 基础600 + 时长奖励 + 里程碑，满冲约900-1000/次 |
...
|------|------|
| 基础 | 每分钟1金币，上限600 |
| >=30分钟 | 20%概率触发随机事件(+10~50) |
| 每小时 | +5金币/小时 |
| 3小时里程碑 | +25 |
| 6小时里程碑 | +50 |
| 9小时里程碑 | +75 |
| 12小时里程碑 | +100 |
| >=2小时 | 10%概率超级冲刺王(+20~50) |
...
| 穷牛一生 | 25 | 随机盲盒 | 娱乐 |
| 巴黎世家 | 50 | +3硬度 | 增益 |
| 巴适得板 | 100 | +20cm,-2硬度 | 增益 |
| 妙脆角 | 70 | 防止一次减半 | 防御 |
| 淬火爪刀 | 100 | 掠夺+10% | 进攻 |
| 致命节奏x5 | 100 | 无debuff打胶 | 增益 |
| 不灭之握 | 200 | +30cm | 增益 |
| 祸水东引 | 250 | 1次转嫁>=50cm伤害 | 防御 |
| 牛牛大自爆 | 250 | 自己归零，伤害top3 | 进攻 |
| 月牙天冲 | 300 | 双方损失50%长度 | 进攻 |
| 劫富济贫 | 300 | 抢首富15%给穷人 | 进攻 |
| 牛牛盾牌 | 500 | 3层护盾免疫攻击 | 防御 |
| 混沌风暴 | 500 | 随机10人触发事件 | 娱乐 |
| 夺牛魔罐头 | 600 | 50%夺全部/20%自清/30%无效 | 高风险 |
| 阿姆斯特朗 | 1000 | +100cm+10硬度 | 增益 |
| 上保险 | 1000 | 10次理赔，损失>=50cm或硬度>=10赔200金币 | 防御 |
| 绝对值！ | 动态 | 负数牛牛花费|长度|金币翻正 | 特殊 |
...
**上保险**（1000金币，10次）
- 损失>=50cm长度 或 >=10硬度 时赔付200金币
- 自残类道具（大自爆、月牙天冲对自己）不赔
