...
### 显示优化
- 全局替换赌博相关文本：赌博→押宝，赌注→彩头，赌徒→押宝牛，赌神→牛神，豪赌→豪拼，对赌→对拼
- 全局替换股市相关文本：股市→妖牛市，股票→妖牛券，股价→牛价，股民→牛民，股评家→牛评家，股灾→牛灾
- 道具「赌徒骰子」更名为「命运骰子」
- 命令「牛牛股市」更名为「牛牛妖市」（子命令不变：购买/出售/持仓/操盘）

## [v4.29.6] - 2026-02-23

### 删除功能
- 下线「牛牛拜年」春节限定活动（年已过，代码保留备用）

## [v4.29.5] - 2026-02-22

### Bug修复
- **停止开冲覆盖金币竞态条件**：`stop_rush` 在 `yield` 后用的是加载时的旧 `data` 对象做第二次全量保存，会把同时进行的股票出售等命令已写入的金币更新覆盖回旧值；改为用 `update_user_data` 原子更新 `is_rushing` / `last_rush_end_time`，不再触碰其他用户数据
...
### Bug修复
- **开冲百分比收益兼容新手**：收益基数改为 `max(金币, 总资产)`，防止新手总资产为0时百分比收益为0（保底固定收益仍兜底）

### 数值调整
- **股市均值回归强度下降**：最大回归概率从85%降至75%（`regression_strength` 上限 0.35→0.25），股价更难被强制拉回基准
- **magnitude 上限提升**：最大幅度系数从3.0提升至4.0，大事件对股价影响更强烈
...
### 数值调整
- **开冲收益改为百分比制**：基础收益从固定「1金币/分钟，上限600」改为「总资产（金币+股票）× 0.5%/小时」，保底仍为固定计算（小资产玩家不受影响），富有玩家收益大幅提升
  - 例：总资产10万，冲12小时 → 基础收益6000金币（原最多600）
  - 结算时显示「总资产 X.XX%」告知收益来源

### 机制调整
- **比划下注改为双向池制**：发起方先入池赌注；赢家取回自己赌注 + 对手赔付（扣税后）；对手金币不足则按现有归零，发起方不会虚空造钱也不会欠债
...
### Bug修复
- **比划下注不再扣到负数金币**：输家金币最多扣到0，赢家只获得输家实际可赔付的金额（扣税后）
- **移除比划下注双重结算bug**：之前赌注会被结算两次（税制结算+1.8倍乘数结算），导致输家被双倍扣款、赢家凭空获得额外金币
- 输家金币不足时显示实际赔付金额提示
...
### Bug修复
- 修复打胶功能崩溃：AFTER_DAJIAO 效果上下文缺少必需的 `nickname` 和 `user_data` 参数
...
### 数值调整
- **含笑五步癫机制重做**
  - 消耗从95%总资产改为**100%总资产**（真正的自爆）
  - 去掉即时掠夺100%属性（过于通胀）
  - 新增：目标第1步损失（19.6%快照值）**转移给发起方**作为回报
  - 第2-5步损失仍然直接销毁
  - 目标资产越高，发起方回报越大，天然鼓励"劫富"
...
### 数值调整
- **含笑五步癫**：发起方立即掠夺目标快照的100%长度、硬度、资产
  - 原本发起方消耗95%资产但无任何收益，属于纯报复道具
  - 现在发起方立即获得目标的长度+硬度+资产（金币形式），使其成为高风险高回报的掠夺道具
  - 目标仍然承受5步×19.6%=98%的持续伤害
...
### Bug修复
- 修复吃瓜群众订阅两个效果均不生效的问题
  - 打胶增长效果：从未触发 `AFTER_DAJIAO`，现已在打胶数据保存后添加触发
  - 道具售价10%金币效果：从未实现，现已在购买道具成功后添加触发（30%概率）
  - 修正描述文案：明确为"道具售价10%金币"
...
- **牛牛拜年 所有人** - 一次性拜年到今日上限并汇总结算
  - 自动查找群内所有可拜年的牛友（已注册、非自己、今天未拜访过）
  - 随机顺序拜访，直到达到每日上限(20次)或拜完所有牛友
  - 特殊事件、集五福等机制与单次拜年完全一致
  - 输出汇总报告：逐条明细、总计收获、触发事件、福卡获取
  - 集五福大奖在批量过程中正确触发，后续不再掉落福卡
...
### Bug修复
- 修复吃瓜群众订阅不生效的问题
  - 原因：打胶结果计算后从未触发 `AFTER_DAJIAO` 效果，导致吃瓜群众订阅永远不会触发
  - 修复：在打胶数据保存后添加 `AFTER_DAJIAO` 触发调用
...
### Bug修复
- 修复比划比划对决中，输家金币为负数时赢家反而亏钱的严重bug
  - 原因：输家负金币余额被当作实际下注金额，导致赢家"赢得"负数金币
  - 修复：移除"金币不足按余额结算"逻辑，统一按原始下注额结算，输家不够就欠债
...
### 新增功能
- **牛牛重锤** (800金币): 指定目标，每个重锤击破5层护盾
  - 不足5层直接清零
  - 支持批量购买：`牛牛购买 22 @目标 数量`
  - 自动限制购买数量（不会浪费多余的锤子）
  - 目标没有护盾时无法购买
...
### 数值调整
- **救市/砸盘/操盘影响力公式改为对数衰减**
  - 旧公式：`2% + 金额/5000 * 2%`（线性，基础2%白送）
  - 新公式：`1% * log2(1 + 金额/1000)`（对数衰减，小额微波动）
  - 1金币: 2% → 0.001%，杜绝白嫖套利
  - 1000: 2.4% → 1%，5000: 4% → 2.6%，10万: 42% → 6.6%
...
### 新增功能
- **牛牛股市 操盘 <金额>** - 所有玩家可用的股市操纵功能
  - 花自己的金币拉盘（正数）或砸盘（负数）
  - 金币直接销毁，不进任何人口袋
  - 影响力公式与管理员救市一致：基础2% + 每5000金币额外2%
  - 无冷却限制，配合持仓可形成"低买→拉盘→高卖"套利玩法
  - 专属玩家操盘文案（区别于管理员的国家队文案）
...
### 数值调整
- **集齐五福大奖加强**
  - 基础奖励：+100cm → **+188cm**, +5硬度 → **+88硬度**, +20000金币 → **+28888金币**
  - 新增财富加成：额外获得当前总资产（金币+股票市值）的**50%**作为金币奖励
  - 负数金币不计入总资产，股票市值按当前股价计算
...
### 新增功能
- **牛牛拜年** - 春节限定互动功能
  - `牛牛拜年 @目标` 给牛友拜年，双方获得随机奖励
  - 拜年者：+1~5cm, +25~100金币, 10%概率+1硬度
  - 被拜者：+2~8cm, +50~150金币, 5%概率+1硬度（被拜访的人多拿一些）
  - 每天最多拜20家，同一牛友每天只能拜1次
  - 10种特殊事件（50%概率触发）：
    - 🐂 牛气冲天、🧧 红包雨、👹 年兽来袭、⭐ 福星高照
    - 🚪 拜错门、💰 财神到、🥟 团圆饭、🔄 牛转乾坤
    - 🧨 爆竹惊牛、💰 压岁钱
...
### Bug修复
- **修复夺牛魔生效后不被扣除的问题**
  - 根因：`shop.consume_item()` 直接写文件，绕过了比划流程的内存缓存，缓存刷新时覆盖了道具消耗
  - 新增 `consume_item` 直接操作缓存数据
- **修复比划下注成功后不加钱的问题**
  - 同样的缓存一致性问题：`shop.modify_coins()` 和 `games.update_user_coins()` 绕过缓存
  - 新增 `modify_coins_cached` 方法，比划流程内所有金币操作改用缓存感知方法
  - 同时修复了比划内保险理赔、围观效应金币奖励等相同问题
...
### Bug修复
- **修复夺牛魔混沌风暴/大自爆保存数据方法不存在的问题**
  - `_save_niuniu_data` → `_save_data`
...
### Bug修复
- **修复夺牛魔不消耗护盾的问题**
  - 夺牛魔攻击有护盾的目标时，现在会正确消耗护盾层数
  - 每层护盾减免10%伤害，最多消耗10层（100%减伤）
...
### 机制调整
- **飞飞机改为百分比机制**
  - 收益/损失从固定数值改为基于总资产（金币+股票市值）的百分比
  - 保持原有事件分布和相对比例
  - 百分比范围控制在合理区间：
    - 低收益：+1.5%~+5%
    - 中等收益：+5%~+8%
...
### Bug修复
- **修复化牛绵掌/含笑五步癫严重运行时bug**
  - 修复 `NiuniuStock(group_id)` 调用错误（应为 `NiuniuStock.get()` 单例模式）
  - 修复 `get_stock_data()` 方法不存在的问题（改用 `get_holdings()` 和 `get_price()`）
  - 修复 `save_stock_data()` 方法不存在的问题（改用 `force_liquidate()`）
  - 修复股票数据键名错误（`shares` 改为 `holdings`）
  - 修复 user_id 类型不一致问题（统一使用字符串）
  - 修复目标股票数据获取方式
  - 影响文件：main.py, niuniu_shop.py, niuniu_effects.py, niuniu_games.py
- **修复注释错误**
  - 计算伤害注释从 24.5% 更正为 19.6%
...
### 新增功能
- **抢劫支持防护道具**
  - 🛡️ 护盾：抵挡抢劫，消耗1层护盾，抢劫者空手而归
  - 🔄 祸水东引：损失>=50金币时触发，抢劫转嫁给随机群友
  - 优先级：护盾 > 祸水东引
...
### 数值调整
- **「含笑五步癫」debuff 重命名**
  - 原名「化骨debuff」改为「含笑五步癫」
  - 触发次数从4次改为5次（走5步）
  - 每次伤害从24.5%改为19.6%（总量保持98%不变）
  - emoji标识从🦴改为【🤪癫】
...
- **化牛绵掌 完全重制**
  ...
  - 化骨debuff机制完全重写：
    - 不再阻止使用「绝对值！」或混沌风暴归零
    - 改为：每次行动都会扣除快照值的24.5%（长度/硬度/总资产）
    - 共触发4次，效果无法被任何道具抵挡
    - 资产扣除含股票：先扣金币，不足时强制清算股票
    - 股票强制清算记为纯损失（不影响股价，无收益）
  - 商城显示动态价格：99%资产=xxx（底价10亿）
  - 排行榜和背包显示🦴化骨标识
  - 触发条件：打胶、比划、抢劫、开冲、飞飞机、买卖股票等行动

### Bug修复
- **股票强制清算统计正确记录**
  - 新增 `force_liquidate` 方法处理化骨强制卖股
  - 正确记录成本和损失，不会导致持仓显示异常
...
### 显示优化
- **战斗大师订阅描述更新**
  - 补充说明：抢劫成功率+5%（该功能早已生效，只是描述未提及）
  - 完整效果：打胶冷却-75%，成功率+10%，比划胜率+5%，抢劫成功率+5%
...
### 新增功能
- **化牛绵掌** (动态定价)：终极打击道具
  - ⚠️ 注意：此版本的机制已在 v4.21.3 中完全重制
  - 代价：消耗99%总资产（金币+股票市值），底价100万金币
  - 效果：将指定目标打到 -100m 长度 和 -100 硬度
  - 施加「化骨debuff」：目标无法使用「绝对值！」翻身，也无法被混沌风暴等意外事件归零
  - 解除条件：通过打胶/比划获胜等正常途径将长度打回正数
  - 用法：`牛牛购买 22 @目标`
